...
`start-demo.ps1` starts the emulators, seeds a demo election + voters, and
...
`127.0.0.1` on the ports above and uses the throwaway project id `sues-d7a7f`,
so no production credentials are required.

## Testing the flows

1. Open http://localhost:5173 and **register a new account**. The
   `onUserCreated` Cloud Function (running in the Functions emulator) automatically
   creates the user document, assigns the `VOTER` role, and adds them to the
   voter roster. You can immediately vote in an active election.
